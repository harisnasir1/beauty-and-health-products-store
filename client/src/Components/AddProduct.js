...
import {Add_Product,Upload_Images,Getall_Categories,Getall_properties} from '../Utils/ApiRoutes'
...
const AddProduct = () => {
  ...
  const handelSubmit=(event)=>{
    ...
    if(datavalidation()){
      console.log("submiting",property)
  ...
  useEffect(()=>{
   ...
   useEffect(() => {
    // Set the default property if properties are available
   
    if(Categories?.length)
      {
        setproduct_category(Categories[0]._id)
      }
  }, [Categories]);
...
setimages(neworder);
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 15.75 3 12m0 0 3.75-3.75M3 12h18" />
      ...
      <h1 className='pl-10 text-custom-black'>Add Product</h1>
    </div>
    <div className='text-black  w-full flex-grow ' style={{flexBasis: '98%'}}>
     ...
     <div className=' flex w-full gap-56'>
      ...
      <div className=' flex flex-col gap-2 '>
     <label className=' text-2xl text-custom-gray font-bold capitalize '>product property </label>
     <select
        className="w-[15vw]  text-white bg-custom-gray outline-none focus:ring-0 border-gray-300 h-12 text-center text-bold text-lg"
       value={property}
      ...
      <label className=' text-2xl text-custom-gray font-bold capitalize'>price in {
        selectedValue==="pak"? "Pkr":selectedValue==="US"?"USD":selectedValue==="UK"?"Pounds":""
      }</label>
      ...
      <div className=' mb-26 flex flex-col gap-3'>
          ...
          <ReactSortable list={images} setList={UpdateImagesOrder}  className='flex flex-wrap gap-2'>
       {
        !!images.length && images.map((link,index)=>(
          <div key={index} className=' flex-grow flex-shrink basis-auto max-h-fit'>
   ...
   <input type='file' className=' hidden cursor-pointer' onChange={UploadImages} multiple/>
      ...
      <button className='bg-custom-gray  w-fit text-xl  text-white rounded-xl cursor-pointer p-2   mt-4 capitalize'>submit</button>
     </form>
